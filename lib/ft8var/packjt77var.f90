...
! These variables are accessible from outside via "use packjt77var":
...
subroutine unpack77var(c77,nrx,msg,unpk77_success,nthr)
!
...
1012 format(i3.2)
     ...
     call unpack28var(n28a,call_1,unpk28_success,nthr) 
     if(.not.unpk28_success .or. n28a.le.2) unpk77_success=.false.
     call unpack28var(n28b,call_2,unpk28_success,nthr)
...
!     endif
     ...
     if(nrx.eq.1) then
       if(dxcall13_set .and. len(trim(dxcall13)).ge.3 .and. hashdx10.eq.n10) then; call_3='<'//trim(dxcall13)//'>'
         elseif(.not.dxcall13_set .and. len(trim(dxcall13)).ge.3  .and. .not.lcommonft8b .and. hashdx10.eq.n10) then
           call_3='<'//trim(dxcall13_0)//'>'
         elseif(.not.dxcall13_set .and. len(trim(dxcall13)).ge.3  .and. .not.lcommonft8b .and. hashdx10.ne.n10) then
           call_3=trim(call_3)
         elseif(.not.dxcall13_set .and. len(trim(dxcall13)).ge.3  .and. lcommonft8b .and. hashdx10.eq.n10)  then
           call_3='<'//trim(dxcall13_0)//'>'
         elseif(.not.dxcall13_set .and. len(trim(dxcall13)).ge.3  .and. lcommonft8b .and. hashdx10.ne.n10)  then
           call_3='<...>'
       endif
     endif
...
2010    format(b28.28,b15.15,b5.5)
        ...
        if (unpk77_success) call save_hash_callvar(call_1,nthr) !### Is this OK here? ###
...
1022 format(i2,i4.4)
     call to_grid6var(igrid6,grid6,unpk77_success)
...
! Type 0.1:  K1ABC RR73; W9XYZ <KH1/KH7Z> -11   28 28 10 5       71   DXpedition special msg
  ...
  call pack28var(w(3),n28b,ntxhash)
  lhashit=.false.; if(ntxhash.eq.1) lhashit=.true.
...
10 i1psuffix=index(w(1)//' ' ,'/P ')
  ...
  call pack28var(c13,n28a,ntxhash)
  c13=bcall_2
  if(w(2)(1:1).eq.'<') c13=w(2)
  call pack28var(c13,n28b,ntxhash)
  ipa=0
  ipb=0
...
1    mult='   '
     ...
     if(mult.ne.'   ' .or. nserial.gt.0) then
        ...
        call pack28var(w(1+itu),n28a,ntxhash)
        call pack28var(w(2+itu),n28b,ntxhash)
        ir=0
